...
# TODO always make sure the file is names FreeFile.exe on startup

#Before anything check if there is a FREEFILE244TEMP.exe exists, if so that means we just updated and we want to delete it
if(os.path.isfile("FREEFILE244OLDTEMP")):
    os.remove("FREEFILE244OLDTEMP")
...
def update_free_file():
    global running, running_thread, code
    if not running:
        # creating a thread to receive the updated file file, while also updating the loading bar, and replacing the current exe
        running_thread = threading.Thread(target=update_free_file_thread, daemon=True)
        running_thread.start()
def update_free_file_thread():
    success = socketConnection.update(pb = progressBar, lb = label)
    time.sleep(1)
    if(success == 1):
        os.rename("Free File.exe", "FREEFILE244OLDTEMP")
        os.rename("FREEFILENEW.exe", "Free File.exe")
    elif(os.path.isfile("FREEFILENEW.exe")):
        os.remove("FREEFILENEW.exe")
...
def rcvFile():
    ...
    if not running:
        ...
        # If the user wants to update, we go through a different seies of events
        if(entry.get() == "update"):
            print("u")
            update_free_file()
            return
